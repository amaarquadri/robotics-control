...
from src.simulation.simulator import simulate, simulate_direct
...
def test_system(controller, x_r_func=None, state_0=None, t_f=10):
    ...
    #                               state_0, t_f)
    t_vals, state_vals = simulate_direct(controller, state_0, t_f)
...
def main():
    phys = CartPole(M=1.994376, m=0.105425, L=0.110996, b=1.6359, g=9.81)
    controller = LinearController(phys, operating_point=None, C=np.array([[1, 0, 0, 0]]),
                                  Q=np.diag([1, 1, 10, 100]), R=np.array([[0.0001]]),
                                  V=0.001*np.identity(4), W=0.001*np.identity(1))

    t_vals, state_vals = test_system(controller, x_r_func=lambda t: [0, 0, 0, 0],
                                     state_0=np.array([0, 0, 0, 0,
                                                       0.1, 0, 0, 0]),
                                     t_f=20)
    for i, x in enumerate(phys.x):
        plt.plot(t_vals, state_vals[2 * i, :], label=f'${to_string(x)}$')
        # plt.plot(t_vals, state_vals[2 * (phys.x_dim + i), :], label=f'$\\hat{{{to_string(x)}}}$')

    # for i, x in enumerate([sp.diff(x, phys.t) for x in phys.x]):
    #     plt.plot(t_vals, state_vals[2 * i + 1, :], label=f'${to_string(x)}$')
        # plt.plot(t_vals, state_vals[2 * (phys.x_dim + i) + 1, :], label=f'$\\hat{{{to_string(x)}}}$')
