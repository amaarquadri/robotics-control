...
def simulate_closed_loop(physics_system, C, controller, V=None, W=None, state_0=None, t_f=10, rtol=1e-3):
    """
    ...
    measurement_noise = gaussian_noise_function(variances=W)
    ...
    def state_derivative(t, state):
        ...
        y = np.dot(C, x) + measurement_noise(t)
...
def simulate_open_loop(physics_system, t_data, u_data, state_0=None, t_f=120):
    if state_0 is None:
        state_0 = np.zeros(2 * physics_system.x_dim)

    def state_derivative(t, x):
        u = np.interp(t, t_data, u_data)
        return physics_system.equations_of_motion_func(x, u)
